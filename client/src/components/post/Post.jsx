...
export default function Post(props) {
      ...
      const fetchUser = async () => {
        try {
          const res = await axios.get(
            `http://${IP}:9000/api/users?userId=${id}`
          );
    ...
    const Save = async() => {
        try {
          const response = await axios.put(`http://${IP}:9000/api/users/${post._id}/save`, {
    ...
    const unSave = async() => {
        try {
          const response = await axios.put(`http://${IP}:9000/api/users/${post._id}/unsave`, {
            userId:decodeToken.id,
          });
          setGuardado(false);
            ...
            <div className="postTop">
                ...
                <div className="postTopRight">
                    ...
                    <>
                        ...
                        <span onClick={handleDelete}>  <DeletePost className="postTopRightIcon" userId={post._id} /></span>
                    </>
                    )}
                </div>
            </div>
